...
function oldQuadraticProbe(elements, len) {
  ...
  let result = elements.map((element) => {
    let res =
      '<div style="display: grid; grid-template-columns: max-content max-content; grid-column-gap: 1rem">';
    let i = 0,
      c1 = 1,
      c2 = 3;
...
function quadraticProbe(elements, len) {
  let n = len ? len : elements.length;
